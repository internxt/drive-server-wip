...
describe('SharingRepository', () => {
  ...
  describe('findAllSharingsByItemIds', () => {
    it('When called with valid itemIds then calls repository with correct parameters', async () => {
      const itemIds = [v4(), v4()];
      ...
      jest.spyOn(sharingModel, 'findAll').mockResolvedValue(expectedSharings);

      const result = await repository.findAllSharingsByItemIds(itemIds, where);
      ...
      expect(result).toEqual(
        expectedSharings.map((shared) => Sharing.build(shared)),
      );
    });

    it('When called with empty itemIds then calls repository with empty array', async () => {
    ...
    it('When repository throws an error then it propagates the error', async () => {
      const itemIds = [v4()];
      const where = { sharedWithType: SharedWithType.WorkspaceTeam };
      ...
      await expect(
        repository.findAllSharingsByItemIds(itemIds, where),
