      ...
      while (!success && retries < maxRetries) {
        try {
          ...
          if (retries < maxRetries) {
            Logger.warn(
              `[RETENTION_POLICY] Batch deletion failed, retry ${retries}/${maxRetries} for user ${userUuid}`,
              error,
            );
            await this.delay(1000 * retries);
          } else {
            Logger.error(
              `[RETENTION_POLICY] Batch failed after ${maxRetries} retries, skipping batch for user ${userUuid}`,
              error,
            );
