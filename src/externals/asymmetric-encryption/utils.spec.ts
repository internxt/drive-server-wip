...
describe('XORhex', () => {
  it('When identical hex strings are XORed, then it should return a string of zeros', () => {
    ...
    const result = XORhex(hexString, hexString);

    expect(Buffer.from(result).toString('hex')).toBe(expectedResult);
  });
  ...
  it('When a fixed example is provided, it should return the expected result', async () => {
    ...
    expect(Buffer.from(xoredMessage).toString('hex')).toEqual(resultHex);
  });

  it('When XORing with a zero-filled hex string, then it should return the original string', () => {
    const a = '12345678';
    const zeroString = '00000000';
    const result = XORhex(a, zeroString);

    expect(Buffer.from(result).toString('hex')).toBe(a);
  });
  ...
  it('When input strings have different lengths, then it should throw an error', () => {
    ...
    expect(() => XORhex(a, b)).toThrow('Can XOR only identical lengths');
  });

  it('When both input strings are empty, then it should return an empty array', () => {
    expect(XORhex('', '')).toStrictEqual(new Uint8Array(0));
  });
});
